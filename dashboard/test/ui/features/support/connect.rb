...
def saucelabs_browser(feature_name)
  ...
  capabilities[:name] = feature_name
...
def get_browser(feature_name)
  if ENV['TEST_LOCAL'] == 'true'
    ...
    saucelabs_browser(feature_name)
  end
end
...
Before do |scenario|
  ...
  if slow_browser?
    browser ||= get_browser(scenario.feature.name)
